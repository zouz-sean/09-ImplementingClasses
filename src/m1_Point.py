...
class Point(object):
    ...
    def move_to(self, x, y):
            self.travel = (math.sqrt((self.x - self.b) ** 2 + (self.y - self.b) ** 2))
            self.x = x
            self.y = y
            self.count = self.count + 1
    def move_by(self, dx, dy):
            self.travel = self.get_distance_from(Point(self.x + dx, self.y + dy)) + self.travel
            self.x = self.x + dx
            self.y = self.y + dy
            self.count = self.count + 1
    ...
    def get_distance_from_start(self):
            self.distance = self.get_distance_from(Point(self.a, self.b))
            return self.distance
    def get_distance_traveled(self):
            return self.travel
    def closer_to(self, Point2, Point3):
        j1 = self.get_distance_from(Point(Point2.x, Point2.y))
        j2 = self.get_distance_from(Point(Point3.x, Point3.y))
        if j1 > j2:
            return Point3
        elif j1 < j2:
            return Point2
